...
const selectElement = document.getElementById('필터색변경');
selectElement.classList.add('body__search__selection_text')
window.addEventListener("scroll", () => {
    const scrollDownLength = window.scrollY;
        if(scrollDownLength > 0) {
            selectElement.classList.remove('body__search__selection_text');
            selectElement.classList.add('selection_text-reverse');
        } else {
            selectElement.classList.remove('selection_text-reverse');
            selectElement.classList.add('body__search__selection_text');
        }
    }
);
