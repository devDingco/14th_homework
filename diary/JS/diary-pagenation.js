...
// 전역 상태
let savedDiaries = JSON.parse(localStorage.getItem("일기들목록")) || []  // 전체 일기
let filteredDiaries = null  // 검색,드롭다운 결과
let 시작페이지 = 1

// 현재일기들배열, 마지막페이지 설정
const 현재일기들배열함수 = () =>  filteredDiaries ?? savedDiaries
const 마지막페이지함수 = () => Math.ceil(현재일기들배열함수().length / 12)

// 현재일기들배열 받아서 -> 일기들그리기
const 일기들그리기 = (클릭한페이지) => {

    const 현재일기들배열 = 현재일기들배열함수()

    if (현재일기들배열.length > 0) {
        const emptyMsg = document.querySelector(".empty-message")
        if (emptyMsg) emptyMsg.remove()

        const page일기목록 = 현재일기들배열.filter((el, index) => {
