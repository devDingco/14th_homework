...
const 사용자인풋 = ({라벨,타입, 플레이스홀더 = "",}) => {
    ...
    <div className="게시물등록_사용자인풋">
     <label htmlFor="사용자인풋"  className="게시물등록_라벨">{라벨}</label>
     <input id="사용자인풋" className="게시물등록_플레이스홀더" type={타입}  placeholder={플레이스홀더} />
    </div>
    )
};
...
const 버튼인풋 = ({보더컬러, 백그라운드컬러, 컬러, 버튼_내용}) => {
    return (
      <button className="버튼인풋" style={{
      border:보더컬러, backgroundColor:백그라운드컬러, color:컬러 }} >{버튼_내용}</button>
    )
};

// const [에러문구, set에러문구] = React.useState("")

export { 사용자인풋, 주소인풋, 사진첨부인풋, 버튼인풋 };