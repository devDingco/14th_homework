...
export default function BoardsComponentWrite(props) {
  ...
  const onClickEdit = async () => {
    try {
      ...
      const myvariables = { boardId: String(params.boardId) };

      if (writer && writer.trim() !== "") myvariables.writer = writer;
      if (title && title.trim() !== "") myvariables.title = title;
      if (contents && contents.trim() !== "") myvariables.contents = contents;

      const result = await 게시글업데이트요청함수({
        variables: {
          boardId: String(params.boardId),
          password: password,
          updateBoardInput: { title: String, contents: String },
        },
        refetchQueries: [
          {
            query: FETCH_BOARD,
      ...
      <div className={styles.게시물등록_container}>
        ...
        <div className={styles.게시물등록_취소and등록하기버튼}>
          ...
          <button
            ...
            onClick={props.isEdit ? onClickEdit : onClickSignup}
            disabled={props.isEdit ? false : 등록버튼비활성화}
          >
