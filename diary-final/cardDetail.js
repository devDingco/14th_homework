document.addEventListener("DOMContentLoaded", () => {
  try {
    ...
    // 모바일 헤더 노출 토글
    const mobileHeader = document.querySelector(".mobile-header-contents");
    ...
    function updateHeaderForViewport() {
      ...
      if (isMobile) {
        ...
        if (brandLink) brandLink.style.display = ""; // 기본 표시
      }
    }
    updateHeaderForViewport();
    window.addEventListener("resize", updateHeaderForViewport);
    window.addEventListener("orientationchange", updateHeaderForViewport);
  ...
  function moveToOldPage() {
    window.document.documentElement("mobile-back-link");
    history.back();
  }
  moveToOldPage();
});
