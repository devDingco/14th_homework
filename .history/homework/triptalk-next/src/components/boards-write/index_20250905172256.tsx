  ...
  const onClickUpdate = async () => {
    ...
    try {
      ...
      const updateData: any = {
        ...
        password: inputPassword, // prompt로 입력받은 비밀번호 사용
        updateBoardInput: {},
      };

      // 비어있지 않은 필드만 업데이트 (부분 업데이트)
      if (title !== '') updateData.updateBoardInput.title = title; // 제목이 입력되었으면 추가
      if (content !== '') updateData.updateBoardInput.contents = content; // 내용이 입력되었으면 추가

      // 서버에 게시글 수정 요청 전송
      const result = await updateBoard({
        // 개선된 방식: 변경된 필드만 전송 (효율적)
  ...
  return (
    <div className="container">
      <nav className={styles.nav}>
        <div>
          <Image src="/icons/logoArea.png" alt="로고" width={100} height={50} />
        </div>
      ...
      <div className={styles.사진첨부컨테이너}>
        ...
        <div className={styles.사진첨부}>
          <button>
            <Image src="/icons/add.png" alt="업로드" width={24} height={24} />
            <br />
            클릭해서 사진 업로드
          </button>
          <button>
