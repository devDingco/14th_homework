...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // 수정된 삭제 함수 - 고유 ID 기반으로 안전하게
  function 일기삭제하기(삭제할ID) {
    console.log('삭제할 ID:', 삭제할ID);

    // 해당 일기를 배열에서 제거 (ID로 구분)
    diaryList = diaryList.filter((일기) => 일기.id !== 삭제할ID);
  ...
  <div class="image-container">
    ...
    <button class="delete-btn" onclick="event.stopPropagation();일기삭제하기('${일기데이터.id}')">
  ...
  submitBtn.addEventListener('click', function () {
    ...
    const diary = {
      id: Date.now() + Math.random(), // 고유 ID 생성 (타임스탬프 + 랜덤)
